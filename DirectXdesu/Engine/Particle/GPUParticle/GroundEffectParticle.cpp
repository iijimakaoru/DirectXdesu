...
void GroundEffectParticle::Update(const Timer* timer, const KMyMath::Matrix4& matView, const KMyMath::Matrix4& matProjection, Emitter* emitter)
{
	...
	/*while (fence->GetCompletedValue() < currentFrameResource->Fence)
	{
		SwitchToThread();
	}*/
...
void GroundEffectParticle::BuildUAV()
{
	KDirectXCommon* directXCommon = KDirectXCommon::GetInstance();
...
void GroundEffectParticle::BuildRootSignature()
{
...
void GroundEffectParticle::BuildPSOs()
{
	ID3D12Device* device = KDirectXCommon::GetInstance()->GetDevice();
	...
	// Graphic
	{
		...
		graphicPSO_->CreateVertexShader(L"Effect/GroundEffect/GroundEffectVS.hlsl", "main");
		graphicPSO_->CreatePixelShader(L"Effect/GroundEffect/GroundEffectPS.hlsl", "main");
		graphicPSO_->CreateGeometryShader(L"Effect/GroundEffect/GroundEffectGS.hlsl", "main");
	}
	...
	// EmitCS
	{
		psoFutures.push_back(std::async(std::launch::async, [&] {
			emitPSO_->CreateShader(L"Effect/GroundEffect/GroundEffectEmitCS.hlsl", "main");
	...
	{
		psoFutures.push_back(std::async(std::launch::async, [&] {
			updatePSO_->CreateShader(L"Effect/GroundEffect/GroundEffectUpdateCS.hlsl", "main");
	...
	{
		psoFutures.push_back(std::async(std::launch::async, [&] {
			copyDrawPSO_->CreateShader(L"Effect/GroundEffect/GroundEffectCopyDrawCountCS.hlsl", "main");
	...
	{
		psoFutures.push_back(std::async(std::launch::async, [&] {
			deadListPSO_->CreateShader(L"Effect/GroundEffect/GroundEffectDeadListInitCS.hlsl", "main");
...
void GroundEffectParticle::BuildFrameResources()
{
	ID3D12Device* device = KDirectXCommon::GetInstance()->GetDevice();
