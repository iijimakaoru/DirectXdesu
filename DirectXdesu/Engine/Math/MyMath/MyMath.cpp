...
Matrix4
    MakeWorld(Matrix4 translation, Matrix4 scaling, Matrix4 rotation) {
	Matrix4 result;
...
Matrix4
    MakeOrthogonalL(float left, float right, float bottom, float top, float near_, float far_) {
	...
	matrix.m[0][1] = matrix.m[0][2] = matrix.m[0][3] = matrix.m[1][0] = matrix.m[1][2] =
	    matrix.m[1][3] = matrix.m[2][0] = matrix.m[2][1] = matrix.m[2][3] = 0.0f;

	return matrix;
}

Matrix4
    MakeOrthogonalR(float left, float right, float bottom, float top, float near_, float far_) {
	float width = 1.0f / (right - left);
	float height = 1.0f / (top - bottom);
	float range = 1.0f / (far_ - near_);
...
Matrix4 MakeLockAt(Vector3& eye, Vector3& target, Vector3& up) {
...
Matrix4 MakePerspective(float fogAngleY, float aspectRatio, float nearZ, float farZ) {
	...
	matrix.m[0][1] = matrix.m[0][2] = matrix.m[0][3] = matrix.m[1][0] = matrix.m[1][2] =
	    matrix.m[1][3] = matrix.m[2][0] = matrix.m[2][1] = matrix.m[3][0] = matrix.m[3][1] =
	        matrix.m[3][3] = 0.0f;
...
float Sin(float sin) { return std::sinf(sin); }

float Cos(float cos) { return std::cosf(cos); }

float Tan(float tan) { return std::tanf(tan); }

float Asin(float sin) { return std::asinf(sin); }

float Acos(float cos) { return std::acosf(cos); }

float Atan(float tan) { return std::atanf(tan); }
...
Vector3 HermiteGetPoint(
    Vector3 p0, Vector3 p1, Vector3 v0, Vector3 v1, float t) {
...
Vector3 TransforNormal(const Vector3& v, const Matrix4& m) {
	Vector3 result = {
...
Vector3 MatrixTransformWDivision(const Vector3& v, const Matrix4& m) {
...
Vector3 CatMullRomSprine(std::vector<Vector3>& points, float t) {
	...
	if (index > 0.0f) {
		p2 = 0.5f * (points[static_cast<size_t>(index + 1.0f)] -
		             points[static_cast<size_t>(index - 1.0f)]);
	} else {
	...
	if (index < length - 2.0f) {
		p3 =
		    0.5f * (points[static_cast<size_t>(index + 2.0f)] - points[static_cast<size_t>(index)]);
	} else {
...
DirectX::XMVECTOR ChangeVector4toXMVECTOR(Vector4 vector4) {
...
Vector4 ChangeXMVECTORtoVector4(DirectX::XMVECTOR vector) {
	Vector4 result;
...
Matrix4 ChangeXMMATRIXtoMatrix4(DirectX::XMMATRIX matrix) {
	Matrix4 result;
...
DirectX::XMMATRIX ChangeMatrix4toXMVECTOR(Matrix4 matrix) {
	DirectX::XMMATRIX result;

	result = DirectX::XMMatrixSet(
	    matrix.m[0][0], matrix.m[0][1], matrix.m[0][2], matrix.m[0][3], matrix.m[1][0],
	    matrix.m[1][1], matrix.m[1][2], matrix.m[1][3], matrix.m[2][0], matrix.m[2][1],
	    matrix.m[2][2], matrix.m[2][3], matrix.m[3][0], matrix.m[3][1], matrix.m[3][2],
	    matrix.m[3][3]);

	return result;
}

Vector3 ChangeXMFloat3toVector3(DirectX::XMFLOAT3 float3) {
	Vector3 result;
...
DirectX::XMFLOAT3 ChangeVector3toXMfloat3(Vector3 vector3) {
