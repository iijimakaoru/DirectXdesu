...
{
	common = AhoSpriteCommon::GetInstance();
...
void BakaSprite::Draw()
{
	common->Draw();

	KDirectXCommon::GetInstance()->GetCmdlist()->IASetVertexBuffers(0, 1, &vbView);

	KDirectXCommon::GetInstance()->GetCmdlist()->DrawInstanced(_countof(vertices), 1, 0, 0);
}
