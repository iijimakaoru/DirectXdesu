...
/// 静的メンバ変数の実体
/// </summary>
const float BillParticleManager::radius = 5.0f;				// 底面の半径
const float BillParticleManager::prizmHeight = 8.0f;			// 柱の高さ
...
void BillParticleManager::InitializeDescriptorHeap()
{
	...
	result = KDirectXCommon::GetInstance()->GetDev()->CreateDescriptorHeap(&descHeapDesc, IID_PPV_ARGS(&descHeap));//生成
	if (FAILED(result)) {
		assert(0);
	}
...
void BillParticleManager::LoadTexture(const wchar_t* fileName)
{
	...
	KDirectXCommon::GetInstance()->GetDev()->CreateShaderResourceView(texbuff.Get(), //ビューと関連付けるバッファ
		&srvDesc, //テクスチャ設定情報
		cpuDescHandleSRV
	);
